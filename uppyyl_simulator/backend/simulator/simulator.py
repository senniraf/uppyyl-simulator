...
class Simulator:
    ...
    @staticmethod
    def get_all_potential_transitions(state):
        ...
        for inst_name, loc in state.location_state.items():
            ...
            for _, edge in loc.out_edges.items():
                select_val_combinations = Simulator.get_select_val_combinations(
                ...
                for select_val_comb in select_val_combinations:
                    ...
                    if edge.sync is None:
                        ...
                        chan_obj: UppaalChan = Simulator.c_evaluator.eval_ast(
        ...
        for i, (inst_name, inst_edges) in enumerate(all_out_edges.items()):
            for edge_data in inst_edges["no_sync"]:  # Get non-synced transitions
                ...
                (
                    loc_urgent,
                    loc_committed,
                ) = Simulator._get_transition_type_from_source_locs(
    ...
    @staticmethod
    def get_select_val_combinations(edge, state):
        select_val_iterators = OrderedDict()
        for select in edge.selects:
            select_var_name = select.ast["name"]
            _, select_var_type = Simulator.c_evaluator.eval_ast(
    ...
    def _get_all_enabled_transitions(self, state, all_pot_trans=None):
        if all_pot_trans is None:
            all_pot_trans = Simulator.get_all_potential_transitions(state=state)
        enabled_transitions = []
        for trans in all_pot_trans:
    ...
    def execute_transition(self, transition):
        ...
        potential_transitions = Simulator.get_all_potential_transitions(
